...
queue()
    ...
    .defer(d3.json, "//somesquares.org/static/json/midwest-topo.json")
    .await(ready);
...
function ready(error, coefs, centroids, topology) {
  features = topojson.feature(topology, topology.objects['midwest-geo']).features;
  
  //Remove the water
  var rem = [533, 519, 305, 257, 65, 6];
